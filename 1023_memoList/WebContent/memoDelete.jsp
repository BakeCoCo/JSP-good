...
<body onload="document.deleteForm.password.focus()">
...
//			삭제할 글번호에 해당되는 글이 테이블에 존재하는가 확인하기 위해 삭제할 글번호에 해당되는 글 한 건을 얻어온다.
...
//			삭제할 글 한 건이 테이블에 존재하는가 검사한다.
			if(rs.next()) {
				SimpleDateFormat sdf = new SimpleDateFormat("yyyy년 MM월 dd일 E요일 HH시 mm분 ss초");
%>

<!-- 삭제할 글이 테이블에 존재하므로 삭제할 글을 화면에 보여주고 비밀번호를 입력받는다. -->
<form action="deleteOK.jsp" method="post" name="deleteForm">

<!-- 삭제할 글번호와 삭제 후 돌아갈 페이지 번호를 hidden으로 폼에 넣어준다. -->
...
//				삭제할 글번호에 해당되는 글이 테이블에 존재하지 않기 때문에 에러 메시지를 출력하고 memoList4.jsp로 돌려보낸다.
				out.println("<script>");
