...
<table width="1000" align="center" border="1" cellpadding="5" cellspacing="0">
	...
	<!-- 공지글이 있으면 공지글의 개수 만큼 반복하며 공지글을 출력한다. -->
	...
	<tr bgcolor="pink">
		...
		<td align="center">
			<c:set var="name" value="${fn:replace(fn:trim(notice.name), '<', '&lt;')}"/>
		...
		<td>
			...
			<c:set var="subject" value="${fn:replace(fn:trim(notice.subject), '<', '&lt;')}"/>
			<c:set var="subject" value="${fn:replace(subject, '>', '&gt;')}"/>
			
			<!-- 제목에 하이퍼링크를 걸어준다. => 하이퍼링크를 클릭하면 조회수를 증가하고 메인글의 내용을 표시한다. -->
			<a href="increment.jsp?idx=${notice.idx}&currentPage=${freeboardList.currentPage}">
				${subject}(${notice.commentCount})
			</a>
		...
		<td align="center">
			<c:if test="${date.year == notice.writeDate.year && date.month == notice.writeDate.month && 
				date.date == notice.writeDate.date}">
				<fmt:formatDate value="${notice.writeDate}" pattern="a h:mm"/>
			</c:if>
			<c:if test="${date.year != notice.writeDate.year || date.month != notice.writeDate.month || 
				date.date != notice.writeDate.date}">
	...
	<!-- request 영역의 freeboardList 객체에서 한 페이지 분량의 글이 저장된 ArrayList(freeboardList)의 내용만 얻어낸다. -->
	<c:set var="list" value="${freeboardList.freeboardList}"/>
	
	<!-- 메인글이 한 건도 없으면 없다고 출력한다. -->
	<c:if test="${list.size() == 0}">
	<tr><td colspan="5">테이블에 저장된 글이 없습니다.</td></tr>
	</c:if>
	...
	<tr>
		<td align="center" colspan="5">
			...
			<c:if test="${freeboardList.startPage > 1}">
				...
				<input type="button" value="-10 page" onclick="location.href='?currentPage=${freeboardList.startPage - 1}'" 
			...
			<!-- 1 페이지 앞으로 -->
			<c:if test="${freeboardList.currentPage > 1}">
				<input type="button" value="-1 page" onclick="location.href='?currentPage=${freeboardList.currentPage - 1}'" 
						title="이전 페이지로 이동합니다."/>
			</c:if>
			<c:if test="${freeboardList.currentPage <= 1}">
			...
			<c:if test="${freeboardList.endPage < freeboardList.totalPage}">
				<input type="button" value="+10 page" onclick="location.href='?currentPage=${freeboardList.endPage + 1}'" 
						title="다음 10 페이지로 이동합니다."/>
				<input type="button" value="end page" onclick="location.href='?currentPage=${freeboardList.totalPage}'" 
