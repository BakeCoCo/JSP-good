...
<!--
	currentPage를 넘겨받아 currentPage에 해당되는 1페이지 분량의 글을 테이블에서 얻어와 request 영역에 저장한 후 1페이지에 분량의
	글을 브라우저 화면에 출력하는 페이지로 넘겨준다.
-->

<%
	request.setCharacterEncoding("UTF-8");
//	이전 페이지에서 넘어오는 화면에 표시할 페이지 번호(currentPage)를 받는다.
//	게시판이 최초로 실행될 때, insertOK.jsp에서 list.jsp가 호출될 때는 currentPage가 넘어오지 않는다. => currentPage는 null 이다.
//	currentPage가 넘어오지 않으면 parseInt() 메소드를 실행했을 때 NumberFormatException이 발생되므로 반드시 예외 처리를 해야한다.
//	이전 페이지에서 넘어오는 currentPage가 없으면 currentPage는 1로 초기화 시켜 사용하고 넘어오는 currentPage가 있으면 넘어온
//	currentPage 값으로 초기화 시켜 사용한다.
	int currentPage = 1;
	try {
...
//	넘어온 검색어가 있으면 카테고리와 검색어를 세션에 저장하고 넘어온 검색어가 없으면 세션에 저장된 카테고리와 검색어를 읽는다.
	if(item != null) {
...
//	테이블에서 브라우저에 표시할 한 페이지 분량의 글을 GuestbookList 클래스 객체에 얻어온다.
...
//	한 페이지 분량의 글과 페이지 작업에 사용할 8개의 변수가 초기화된 객체를 request 영역에 저장한다.
	request.setAttribute("guestbookList", guestbookList);
//	글을 입력할 때 엔터키를 눌러 줄을 바꿔 입력한 경우 브라우저에서 <br/> 태그로 바꿔 출력하기 위해서 request 영역에 "\r\n"를 저장한다.
